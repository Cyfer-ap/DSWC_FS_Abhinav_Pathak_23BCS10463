...
class VideoCache<K, V> extends LinkedHashMap<K, V> {
    ...
    protected boolean removeEldestEntry(Map.Entry<K, V> eldest) {
        if (size() > capacity) {
            System.out.println("  [EVICT] Cache full — evicting LRU entry: \"" + eldest.getKey() + "\"");
            return true;
        }
...
public class StreamFlixCache {
    public static void main(String[] args) {
        ...
        System.out.println("\n--- Verifying eviction ---");
        System.out.println("  'The Matrix' in cache? " + cache.containsKey("The Matrix"));
        System.out.println("  'Inception' in cache?  " + cache.containsKey("Inception"));
